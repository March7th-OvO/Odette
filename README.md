...
仅管理 `march7th-assets` 桶的 `image/` 命名空间。`image/` 是目录浏览器根目录，使用 R2 `prefix + delimiter: "/"` 将扁平 Key 按层级展示；不创建数据库目录记录，也不提供整目录删除或重命名。新上传保留并规范化原文件名（例如 `Furina Wallpaper.JPG` → `furina-wallpaper.jpg`）；当前目录重名时自动追加六位短 hash（例如 `furina-wallpaper-a83f21.jpg`），所有写入均使用条件写，绝不覆盖已有对象。删除继续兼容 UUID Key 和 `image/banner.webp`、`image/march7th/avatar.png` 等旧路径，无需迁移现有对象。目录前缀和删除 Key 都限制在 `image/` 内、UTF-8 最长 1024 字节，不接受空路径段、`.` / `..` 路径段、反斜杠或控制字符。HTTP 接口路径仍为 `/api/images`。
...
- 原文件名、MIME 保存在 R2 元数据；Key 使用当前目录前缀和规范化后的可读文件名，重名时追加六位短 hash。条件写入防止覆盖已有对象。
...
| DELETE | `/api/images` | JSON：`{"key":"image/<directory>/<filename>.png"}` |
