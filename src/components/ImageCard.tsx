...
export function ImageCard({ image, onPreview, onDelete, selected = false, onToggle }: { image: ImageItem; onPreview: () => void; onDelete: () => void; selected?: boolean; onToggle?: () => void }) {
  return <article className="image-card" data-selected={selected}>
    {/* Keep selection separate from the preview button for keyboard and touch access. */}
    {onToggle && <label className="image-select"><input type="checkbox" checked={selected} onChange={onToggle} aria-label={`选择 ${image.originalName}`}/></label>}
    <button className="thumbnail" onClick={onPreview} aria-label={`预览 ${image.originalName}`}><img src={image.thumbnailUrl} alt={image.originalName} loading="lazy"/><span className="image-format">{image.contentType.split('/')[1]?.toUpperCase()}</span><span className="expand"><Maximize2 size={17}/></span></button>
